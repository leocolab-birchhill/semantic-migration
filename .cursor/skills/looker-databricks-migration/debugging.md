...
|-------|---------|--------|
| Auth / infra | `fetch failed`, 401, token expired | Human: `npm run auth:databricks`, restart worker |
...
| Wrong metric-view source | `draft/metric_view.yaml` `source:` → job sql_view |
...
# write edge-case note
```

Do **not** call OpenAI diagnose. Do **not** invent a parallel repair API.
